  ...
  return (
    <Dialog open={scale} fullWidth>
        ...
        <div className='grid gap-5 p-5' >
            ...
            <Inputs name='New Rate (new rate will be set for all the previous stock)'  onchange={(e:any)=>{setUpdate((prev)=>{return{...prev,rate:e.target.value}})}} type={'number'} value={update.qty} key={rateInputKey} />         
            <div className='flex gap-5'>

                <SolidButton color='black' innerText='ADD STOCK' onClick={()=>{console.log(update)}} key={addStockButtonKey}/>
                <SolidButton color='error' innerText='CANCEL' onClick={()=>{setUpdate({
                    _id:'',
                    qty:0,
                    rate:0
                });close(false)}} key={addStockButtonKey}/>
            </div>
        </div>

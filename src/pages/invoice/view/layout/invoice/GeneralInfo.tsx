...
{    invoiceKeys.map((index,i:number) => {
        ...
        if(index==='state')return<>
           <div className='flex gap-5   w-full pl-2 border-b border-r  border-black text-table'>
        ...
        if(i<8){
        return<>
          <div className='grid   grid-cols-2 pl-2 border-b  border-r  border-black text-table'>
            <div className=''>{replaceUnderscoresWithSpaces(index)}:</div>
            <div className=' pr-2'>{invoice[index]}</div>
          </div>
        </>
        }else{
          return <div className='grid   grid-cols-2 pl-2 border-b  border-black text-table'>
          <div className=''>{replaceUnderscoresWithSpaces(index)}:</div>

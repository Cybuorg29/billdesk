...
const InputInfo = ({ invoice, setInvoice }: Props) => {
  ...
  return (
    <div className='grid grid-cols-2 grid-rows-4 border border-b-0 h-full text-sm'>
      {
        invoiceKeys.map((index) => {
          if (index === 'total_Tax' || index === 'products' || index === 'gst_On_Reverce_Charge' || index === 'billed_To' || index === 'billed_From' || index === 'shipped_To' || index === 'discount' || index === 'grand_Total' || index === 'state_Code') return
